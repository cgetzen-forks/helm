...
func TestRecordsAdd(t *testing.T) {
	rs := records([]*record{
		newRecord("rls-a.v1", releaseStub("rls-a", 1, 10, "default", rspb.Status_SUPERSEDED)),
		newRecord("rls-a.v2", releaseStub("rls-a", 2, 10, "default", rspb.Status_DEPLOYED)),
	})
	...
	var tests = []struct {
	...
	}{
		{
			...
			"rls-a.v3",
			false,
			newRecord("rls-a.v3", releaseStub("rls-a", 3, 10, "default", rspb.Status_SUPERSEDED)),
		},
		{
			"add already existing key",
			"rls-a.v1",
			true,
			newRecord("rls-a.v1", releaseStub("rls-a", 1, 10, "default", rspb.Status_DEPLOYED)),
		},
	}
