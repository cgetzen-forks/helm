...
func TestMemoryCreate(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			releaseStub("rls-c", 1, 10, "default", rspb.Status_DEPLOYED),
			false,
		},
		{
			"create should fail (release already exists)",
			releaseStub("rls-a", 1, 10, "default", rspb.Status_DEPLOYED),
			true,
		},
	}
...
func TestMemoryUpdate(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			"rls-a.v4",
			releaseStub("rls-a", 4, 10, "default", rspb.Status_SUPERSEDED),
			false,
		},
		{
			"update release does not exist",
			"rls-z.v1",
			releaseStub("rls-z", 1, 10, "default", rspb.Status_DELETED),
			true,
		},
	}
